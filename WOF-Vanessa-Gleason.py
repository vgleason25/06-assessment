...
def read_dictionary_file():
    ...
    # Store each word in a list. = my_dictionary SUCCESS then... didnt work
...
def get_word():
    ...
    round_word = random.choice(dictionary)#choose random word from dictionary SUCCESS
    raw_round_word = round_word
...
def wof_round_setup(round_num): #getting 1st player, get_word() word set up
    ...
    # set first player Return the starting player number (random) SUCCESS
...
def guess_letter(letter, player_num, round_num): # GENERAL defines if approved letter(consonant or vowel) is in the puzzle
    global round_word
    global round_underscore_word 
    global counts
    strcount = ''
...
def final_guess_letter(letter, player_num, round_word): #final game specific  
    global round_underscore_word 
...
def buy_vowel(player_num, round_num): #buys, selects, and evaluates vowel  
    global players
    global vowels
...
def wof_round(player_num, round_num):
    ...
    # TODO Print round_status with string.format, tell people the state of the round as you are leaving a round.

# intro, gives letters, asks for letters 
def wof_final_round(player_num, round_num, round_word):
    ...
    print(final_round_text) # Print out instructions for that player
