...
def wof_round_setup(round_num): #getting 1st player, get_word() word set up
    ...
    time.sleep(2) # pause again for more dramatic effect!
    print('Our starting player will be Player {} \n'.format(init_player))
...
def guess_letter(letter, player_num): # defines if approved letter(consonant or vowel) is in the puzzle # parameters:  take in a letter guess and player number
    #EVALUATION TOOL FROM MY WORD GAME
    global round_word
    global round_underscore_word 
    global counts
    #global strcount
...
def final_guess_letters():# removed 'player_num' from () # defines if given and choosen letters are in the puzzle:
...
def solve_puzzle(player_num, round_num):
    ...
    print(round_word)
    time.sleep(2) # pause again for more dramatic effect!
    if answer == round_word: #TODO i dont think this is correct
