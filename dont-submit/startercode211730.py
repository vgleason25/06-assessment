...
def guess_letter(letter, player_num): # GENERAL defines if approved letter(consonant or vowel) is in the puzzle # parameters:  take in a letter guess and player number
    global round_word
    global round_underscore_word 
    global counts
    #global strcount
...
def wof_final_round(player_num, round_num, round_word):
    ...
    print(f'The word: {round_underscore_word}')
    answer = input('You have 5 seconds to enter the complete word: ')
    timer()
    if answer == raw_round_word: 
        ...
        print("\nOh! You didn't get it. Thanks for playing. \n Goodbye!")
